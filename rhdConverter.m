function [sctHeader, pdData, pdTTL] = rhdConverter(strRHDFile, strSaveDir, bTTL,subjNumber,numSubjects,numChannels)
% clear all; 
[strSavePath, strName, ~] = fileparts(strSaveDir); 
...
%% read and convert RHD to MATLAB variables
function [sctHeader, pdData, pdTTL] = readIntanRHD2000File(strRHDFile, strSaveDir, fWaitbar,subjNumber, numSubjects, numChannels)
...
if magic_number ~= hex2dec('c6912702')
    errordlg('Unrecognized file type.', 'File Error');
end
...
waitbar(0.1, fWaitbar, sprintf('Reading RHD data file, Version %d.%d...', ...
...
if num_amplifier_channels ~= (numSubjects*numChannels)
    close(fWaitbar);
    delete(fWaitbar);
...
if (data_present)
    ...
    amplifier_data = zeros(numChannels, num_amplifier_samples);
    ...
    for i=1:num_data_blocks
        ...
        if (num_amplifier_channels > 0)
            temp = fread(fid, [60, numChannels*numSubjects], 'uint16')';
            amplifier_data(:,amplifier_index:(amplifier_index+59)) = temp(((subjNumber*numChannels)-numChannels)+1:subjNumber*numChannels,:);
            temp=[];
        end
        ...
        if (fraction_done >= percent_done)
            waitbar(percent_done/200 + 0.1, fWaitbar, sprintf('Readig data from file... %d%% done.', percent_done));
%             fprintf(1, '%d%% done...\n', percent_done);
...
if (data_present)
    waitbar(0.65, fWaitbar, 'Parsing data...'); 
%     fprintf(1, 'Parsing data...\n');
    ...
    % end
    % 
%waitbar(0.7, fWaitbar, 'Saving MAT-file data...'); 
fprintf(1, 'Done!  Elapsed time: %0.1f seconds\n', toc);

sctHeader.sctAmplifierChannels  = amplifier_channels(1,((subjNumber*numChannels)-numChannels)+1:subjNumber*numChannels);
sctHeader.sctBoardADCChannels   = board_adc_channels; 
sctHeader.sctBoardDigInChannels = board_dig_in_channels;
sctHeader.sctFrequencyParameters = frequency_parameters; 
sctHeader.sctNotes              = notes;
%sctHeader.sctSpikeTriggers      = spike_triggers;
sctHeader.sctSpikeTriggers      = spike_triggers(1,((subjNumber*numChannels)-numChannels)+1:subjNumber*numChannels);
sctHeader.nChannelNum           = length(amplifier_channels(1,((subjNumber*numChannels)-numChannels)+1:subjNumber*numChannels));
sctHeader.dSamplingRate         = frequency_parameters.amplifier_sample_rate; 
sctHeader.dDuration             = record_time; 

pdData = amplifier_data'; 
pdTTL = board_dig_in_data'; 
pdADC = board_adc_data';

save(strSaveDir, 'pdData', 'pdTTL', 'pdADC', 't_amplifier', 't_board_adc', 't_dig', 'sctHeader', '-v7.3');  

clear amplifier_data board_dig_in_data board_adc_data t_amplifier t_board_adc t_dig; 
% if (data_present)
