function [sctHeader, pdData] = edfConverter(strEDFFile, strSaveDir)

f = waitbar(0, 'Converting...', 'Name', 'Converting EDF file...');

[sctEDFHeader, record] = edfreadUntilDone(strEDFFile, f); 

pdData = record';
clear record; 

nChannels = sctEDFHeader.ns;
fs = sctEDFHeader.frequency(1,1);
for ii = 1:nChannels
    time = strcat(sctEDFHeader.startdate,'.',sctEDFHeader.starttime);
    formatIn = 'dd.mm.yy.HH.MM.SS';
    starttime = datenum(time,formatIn);
    time = datevec(time,formatIn);
    endtime = addtodate(datenum(starttime),size(pdData,2)/fs,'second');
    trdata(1,ii).timestamp = [starttime endtime];
end
% sbuf = data(1,:)';

sctHeader.strPatientID  = sctEDFHeader.patientID; 
sctHeader.strRecordID   = sctEDFHeader.recordID;
sctHeader.strStartDate  = sctEDFHeader.startdate;
sctHeader.strStartTime  = sctEDFHeader.starttime;
sctHeader.strEndTime    = endtime;
sctHeader.trdata        = trdata; 
sctHeader.nChannelNum   = sctEDFHeader.ns; 
sctHeader.dSamplingRate = sctEDFHeader.frequency; 
sctHeader.dDuration     = size(pdData, 1) / sctEDFHeader.frequency(1); 

waitbar(0.75, f, 'Saving data... '); 

save(strSaveDir, 'pdData', 'sctEDFHeader', 'sctHeader', '-v7.3');  
% if isempty(strSaveDir)
%     [strSaveDir, strName, ~] = fileparts(strEDFFile); 
%     save(fullfile(strSaveDir, strName), 'pdData', 'sctEDFHeader', 'sctHeader', '-v7.3');     
% else
%     [~, strName, ~] = fileparts(strEDFFile); 
%     save(fullfile(strSaveDir, strName), 'pdData', 'sctEDFHeader', 'sctHeader', '-v7.3');     
% end
...
function [hdr, record] = edfreadUntilDone(fname, f, varargin)
...
waitbar(0.25, f, 'Reading requested records...'); 
