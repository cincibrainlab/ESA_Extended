function [EEGOUT, com] = EEGLABImportData(strSaveName, pdData, dSamplingRate)
...
com = '';
EEGOUT = eeg_emptyset;
% load('D:\Codes\EEG Analysis for Binder Lab\EEG Analyzer GUI\pop_importdata_results.mat');
% clear uilist varargin; 

% construct loading parameters (by LJZ 20191116)
% ----------------------------
results = cell(1, 15);

results{1} = 1;
results{2} = 'pdData';
results{3} = strSaveName;
results{4} = num2str(dSamplingRate);
results{6} = '0';
results{8} = '0';
results{10} = '0';
...
EEGOUT = EEGLABEditSet(EEGOUT, args{:});
com    = sprintf( 'EEG = pop_importdata(%s);', vararg2str(args) );

end


function [EEGOUT, com] = EEGLABEditSet(EEG, varargin)
    ...
    switch lower(curfield{1})
                         ...
                         if isstr(varname) & exist( varname ) == 2
							...
							try, EEGOUT.icaweights = load(varname, '-ascii');
							 ...
							 if isempty(varname) 
								 EEGOUT.icaweights = [];
							 elseif isstr(varname)
								 EEGOUT.icaweights = evalin('caller', varname, 'fprintf(''pop_editset() warning: variable ''''%s'''' not found, ignoring\n'', varname)' );
								 EEGOUT.icawinv = [];
                             else
        ...
        case 'icachansind', varname = getfield(g, {1}, curfield{1});
							 if isempty(varname) 
								 EEGOUT.icachansind = [];
							 elseif isstr(varname)
								 EEGOUT.icachansind = evalin('caller', varname, 'fprintf(''pop_editset() warning: variable ''''%s'''' not found, ignoring\n'', varname)' );
                             else
                         ...
                         if isstr(varname) & exist( varname ) == 2
                            ...
                            try, EEGOUT.icasphere = load(varname, '-ascii');
							 ...
							 if isempty(varname) 
								 EEGOUT.icasphere = [];
							 elseif isstr(varname)
								 EEGOUT.icasphere = evalin('caller', varname, 'fprintf(''pop_editset() warning: variable ''''%s'''' not found, ignoring\n'', varname)' );
								 EEGOUT.icawinv = [];
                             else
  								 EEGOUT.icaweights = varname;
								 EEGOUT.icawinv = [];                                 
	    ...
	    case 'data'    , varname = getfield(g, {1}, curfield{1});
                         ...
                         elseif isstr(varname)
                             ...
                             %--------------------
                             try 
                                 res = evalin('caller', ['exist(''' varname ''') == 1']);
                             catch
                             ...
                             warning off;
                             try,
                                 testval = evalin('caller', ['isglobal(' varname ')']);
                             catch, testval = 0; end;
                             if ~testval
                             ...
                             evalin('caller', comglobal);
                             eval(comglobal);
                             eval( ['EEGOUT.data = ' varname ';' ]);
                             try, evalin('caller', commandrestore); catch, end;
                             warning on;
                         else 
                             EEGOUT.data = varname;
                             if ndims(EEGOUT.data)<3 & size(EEGOUT.data,1) > size(EEGOUT.data,2), EEGOUT.data = transpose(EEGOUT.data); end;
